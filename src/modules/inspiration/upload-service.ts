...
export class UploadService {
  ...
  /** 上传完成后校验存储对象并落为 ready 素材，返回可读签名 URL 供前端预览。 */
  async complete(ownerId: string, uploadId: string) {
    ...
    if (asset.status === "ready") return { ...asset, url: await this.storage.createDownload(asset.objectKey, 86_400) };
    ...
    return { ...asset, status: "ready" as const, url: await this.storage.createDownload(asset.objectKey, 86_400) };
  }
}
